...
class Builder:
    ...
    def _execute(self, command: str, show_output: bool = True, show_what_is_being_executed: bool = False) -> None:
        if show_what_is_being_executed: print(f"Выполняется: {command}")
        try:
            process = subprocess.Popen(command, shell=True, text=True, executable="/usr/bin/bash", stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            if process.returncode != 0:
                print(f"Произошла ошибка в время выполнения команды! Код ошибки: {process.returncode}.")
                exit(1)
        except subprocess.CalledProcessError:
            print("Произошла ошибка в время выполнения команды!")
            exit(1)
