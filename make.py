...
parser = ArgumentParser(prog="Secux Linux Builder", description="Program for building ISO image of Secux Linux distribution // Программа для сборки ISO образа дистрибутива Secux Linux")
parser.add_argument('-r', '--russian', help="использовать русский язык в GUI", action="store_true", default=False)
...
if args.install_all_dependencies:
    subprocess.run(["/usr/bin/pacman", '-Syu', '--needed', '--noconfirm', 'archiso', 'python-pip', 'python-pillow', 'git', 'bash', 'rsync', 'python-requests', 'python-packaging', 'python-darkdetect'], check=True)
    subprocess.run(['/usr/bin/pip', 'install', 'customtkinter', '--break-system-packages'], check=True)
    exit(0)
...
try:
    ...
    print(f"[ERROR] CustomTkinter {lang.not_installed_use}: pip install customtkinter")
    error = True

try:
    from tkinter import filedialog
except ModuleNotFoundError:
    print(f"[ERROR] Tkinter {lang.not_installed_use}: pacman -Sy tk")
    error = True

try:
    from requests import get
except ModuleNotFoundError:
    print(f"[ERROR] Requests {lang.not_installed_use}: pacman -Sy python-requests")
    error = True

try:
    from PIL import Image
except ModuleNotFoundError:
    print(f"[ERROR] Pillow {lang.not_installed_use}: pacman -Sy python-pillow")
    error = True

if error: 
    print(f"{lang.missing_deps}: --install-all-dependencies")
    exit(1)
...
class Notification(CTkToplevel):
    def __init__(self, message: str, title: str = "Error", icon: str = "warning.png", message_bold: bool = False, exit_btn_msg: str = "Exit"):
        if args.cli:
            return
        super().__init__()
        self.title(title)
        img = Image.open(f'{WORKDIR}/images/{icon}')
...
class App(CTk):
    def __init__(self):
        ...
        if args.cli:
            print(f"[INFO] {lang.cli_on}.")
            self.bin = args.bin
            self.output = args.output
            self.commands = []

            if not os.path.isdir(f"{WORKDIR}/releng"):
                print(f"[ERROR] {lang.releng_error}.")
                return
            
            if not self._check_internet_connection():
                print(f"[ERROR] {lang.internet_error}.")
                return
            
            if not self._check_dependencies():
                print(f"[ERROR] {lang.not_installed_deps}.")
                return
            ...
            if args.offline:
                ...
                if not self.__check_offline_repo():
                    print(f"[WARNING] {lang.offline_repo_autocreate}.")
                    self._execute(f"/usr/bin/echo \"[WARNING] {lang.offline_repo_autocreate}.\"")
                    self.__update_offline_repo()
        ...
        try:
            ...
            print(f"[ERROR] {lang.scaling_error}")
            scaling = 100
        ...
        label = CTkLabel(self, text="Secux Linux Builder")
        ...
        # Default value must be current language. 
        language_label = CTkLabel(self, text="Language // Язык")
        self.language_menu = CTkOptionMenu(self, values = ["English", "Русский"])

        scaling_label = CTkLabel(self, text=lang.scaling)
        self.scaling_menu = CTkOptionMenu(self, values=["80%", "100%", "125%", "150%", "200%"], command=self.__ui_scaling_handler)
        ...
        language_label.grid(row=7, column=1, padx=15, pady=5, sticky="nsew")
        self.language_menu.grid(row=7, column=0, padx=15, pady=5, sticky="nsew")
        scaling_label.grid(row=8, column=1, padx=15, pady=5, sticky="nsew")
        self.scaling_menu.grid(row=8, column=0, padx=15, pady=5, sticky="nsew")

        build.grid(row=10, column=0, columnspan=2, padx=15, pady=5, sticky="nsew")
